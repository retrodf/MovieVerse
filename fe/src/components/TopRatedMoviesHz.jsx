...
const TopRatedMovies = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const response = await fetch(`${URL}/movies/toprated`);
        if (!response.ok) {
  ...
  return (
    <Container className="text-white">
      ...
      <div className="pagination-container">
        ...
        {currentPage > 2 && <Button onClick={() => paginate(1)} className="pagination-btn">1</Button>}
        ...
        {currentPage < totalPages - 1 && <Button onClick={() => paginate(totalPages)} className="pagination-btn">{totalPages}</Button>}
